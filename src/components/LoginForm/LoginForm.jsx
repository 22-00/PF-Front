...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    await axios
      .post("pf-back-production-b670.up.railway.app/user/login", { email, password })
      .then((response) => {
  ...
  const onSuccess = async (response) =>{
    ...
    try {
      const res = await axios.post('pf-back-production-b670.up.railway.app/user/create', user);
      if (res.data.success) {
    ...
    await axios.post("pf-back-production-b670.up.railway.app/user/create", user)
      .then((res) => {
        if (res) {
          const id = res.data.id
          localStorage.setItem("userId", id)
